"""
Section 1 — Trending Fields of Research (by Grant Activity).

Momentum cards comparing the current vs prior grant window,
...
def _build_sparkline(df_exploded: pd.DataFrame, division: str) -> go.Figure:
    ...
    if df.empty:
        ...
        yearly = df.groupby("year")["grant_id"].nunique().rename("count")
        counts = (
...
def render_trending_cards(
    ...
    st.subheader("Trending Fields of Research (by Grant Activity)")
    st.caption(
        f"Grant momentum: current window {current_start}–{current_end} "
            ...
            st.markdown(
                ...
                <div style="border:1px solid #e5e7eb; border-radius:8px;
                  ...
                  <div style="font-size:11px; color:#6b7280; margin-bottom:2px;">
                    ...
                    Preceding three-year period: {row["prior_count"]:,} grants
                  </div>
                </div>
                """,
